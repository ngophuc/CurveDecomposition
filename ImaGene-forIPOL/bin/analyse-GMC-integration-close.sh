...
usage() {
    echo "$0 <prefix> <shape> ..."
    echo "    Analyse the quality of GMC curvature estimation on a given shape <shape> by reintegrating this estimated geometric information into a curve in the plane. Close the estimated curve."
...
cat ${PREFIX}.geom | integrate_shape -curv_idx 4 -cabs_idx 9 -t0 1.57079632679489661922 \
    | sdp2pts -centering -close > ${PREFIX}-int.pts
